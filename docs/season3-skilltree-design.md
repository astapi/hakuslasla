...
### 3.5 吸収・耐久クラスタ改善メモ（2026-06-23）

UberUberボス、とくにクラーケンの10連撃やフリーズ中の被弾に対して、単純なDEFだけでは耐久スコアが伸びにくい。DEFは `def / (def + 500)` 型の軽減なので、3000〜4000以降は伸びが鈍る。ただしUberツリーの `def_hp_to_atk` を取るビルドでは、過剰DEFがATKにも変換されるため価値が残る。

今回の改善では、ペット系に寄っていた継戦能力を全クラスへ分散するため、クラス間の円周ノードから4ノードの吸収・耐久小リングを6個追加した。

- 追加した主な効果: `hp_on_hit`、`damage_defer_pct`、`lifestealPct`、毒/発火/クリティカル吸収。
- `lifestealPct` はパッシブノード用のライフスティール%。火力が上がるほど耐久も伸びるため、後半装備と相性がよい。
- 防御は鎧とアクセサリーでDEF/被ダメ軽減を確保し、武器・手袋などはATK/HP/攻撃速度/サブ状態異常率へ寄せる想定。
- UberUberでは、Uber終焉の甲冑の被ダメ軽減とUber終焉の王冠の高ATK/DEFは前提級装備として扱ってよい。

軽量GAでは各クラスが吸収・耐久クラスタを取得するようになったが、正規ツリー制約ではまだUberUberクラーケン安定突破には届いていない。次の調整候補は、連撃耐性、ダメージ遅延、HIT時回復、ライフスティール%の増量と、GA評価関数で防御上位個体をより残すこと。
...
- **レベルキャップ80化**：初期状態から `MAX_LEVEL=80`。終焉の地クリアはレベル上限解放ではなく、異次元ラッシュなどエンドコンテンツ解放のみを担当する。
