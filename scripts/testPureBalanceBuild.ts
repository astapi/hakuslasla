...
function createUberItem(
  ...
  return {
    ...
    atk,
    def,
    instanceId: `pure_${id}_${itemCounter++}`,
    mods,
  };
}

/**
 * パターン1: Goblinブーツ版（回復重視）
 */
function createPureBalanceGoblin(): { name: string; description: string; equipment: Equipment; passiveNodes: string[] } {
  const passiveNodes = [
    'start',
    // vamp系（20ノード）- HIT時回復
  ...
  return {
    name: '純粋バランス（Goblin版）',
    description: '毒なし / HIT時回復160 / AS+128% / HP回復重視',
    equipment,
    passiveNodes,
  };
}
...
/**
 * パターン2: Banditブーツ版（クリティカル重視）
 */
function createPureBalanceBandit(): { name: string; description: string; equipment: Equipment; passiveNodes: string[] } {
  const passiveNodes = [
    'start',
    // vamp系（20ノード）
    'vamp_1', 'vamp_2', 'vamp_3', 'vamp_4', 'vamp_5',
  ...
  return {
    name: '純粋バランス（Bandit版）',
    description: '毒なし / HIT時回復160 / クリ率93% / AS+128%',
    equipment,
    passiveNodes,
  };
}
...
const BUILD_PATTERNS = [
  createPureBalanceGoblin(),
  createPureBalanceBandit(),
];
