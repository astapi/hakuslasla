...
function createUberItem(
  ...
  return {
    ...
    atk,
    def,
    instanceId: `double_${id}_${itemCounter++}`,
    mods,
  };
}
 ...
 * 双撃の指輪ビルド
 */
function createDoubleStrikeRingBuild(): { name: string; description: string; equipment: Equipment; passiveNodes: string[] } {
  const passiveNodes = [
    'start',
    // vamp系（20ノード）
...
console.log('Uber 双撃の指輪テスト');
console.log(`Level: ${level} / Runs: ${runs} / Seed: ${seed}`);
console.log('='.repeat(80));
console.log('');
console.log('【新アイテム】');
console.log('Uber 双撃の指輪');
console.log('  効果: クリティカル時に追撃（ATK×0.5の追加ダメージ）');
console.log('  特徴: HIT時効果が2回発動（HIT時HP回復が2倍）');
console.log('');
...
for (const pattern of BUILD_PATTERNS) {
  ...
  console.log('【主要MOD効果】');
  console.log(`  HIT時HP回復: +${modEffects.hpOnHit} (クリ時は×2 = ${modEffects.hpOnHit * 2})`);
  ...
  const defReduction = 600 / (600 + 500);
  const baseDmg = Math.floor(finalStats.atk * (1 - defReduction));
  const critDmg = Math.floor(baseDmg * 3.0);
  const followUpDmg = Math.floor((finalStats.atk * 0.5) * (1 - defReduction));
  const totalCritDmg = critDmg + followUpDmg;
  const avgDmg = totalCritDmg * (modEffects.criticalChance / 100) + baseDmg * (1 - modEffects.criticalChance / 100);
  const attackSpeed = 1.0 * (1 + modEffects.attackSpeedPct / 100);
  const dps = avgDmg * attackSpeed;

  console.log(`  基本ダメージ: ${baseDmg}`);
  console.log(`  クリダメージ: ${critDmg} (×3.0)`);
  console.log(`  追撃ダメージ: ${followUpDmg} (ATK×0.5)`);
  console.log(`  クリ時合計: ${totalCritDmg}`);
  console.log(`  期待値: ${Math.floor(avgDmg)}`);
  console.log(`  攻撃速度: ${attackSpeed.toFixed(2)}倍`);
  console.log(`  理論DPS: ${Math.floor(dps)}`);
  console.log('');
